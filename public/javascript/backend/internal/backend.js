...
function addField(beginX, beginY, sizeX, sizeY) {
    for (var x = 0; x < sizeX; x++) {
        for (var y = 0; y < sizeY; y++) {
            var field = game.field[beginX + x][beginY + y];
        }
    }
}
...
function makeMove(move) {
    var result = [];
    var field1 = game.field[move.x][move.y];
    var field2 = game.field[move.x][move.y];
...
function isGameFinished() {
    game.field.forEach(function (item, index) {
        if (!item.right || !item.left || !item.top || !item.bot)
            return false;

...
function canvasScroll(e) {
    if (e.wheelDeltaX != 0)
        return; //unwanted mouse wheel action
    var newZoom = zoom;
    if (e.deltaY > 0) {
        newZoom = newZoom / 1.5;
    }
    ...
    if (newZoom > 15 || newZoom < 1)
        return; //zoom outside of allowed range
    var oldMousePosRel = getRelativeMousePos(e);
    zoomFunct(newZoom);
    var newMousePosRel = getRelativeMousePos(e);
...
function getRelativeMousePos(e) {
    var rect = gameCanvas.canvas.getBoundingClientRect(), // abs. size of element
    ...
    var xPos = (e.clientX - rect.left) * scaleX; // scale mouse coordinates after they have
    var yPos = (e.clientY - rect.top) * scaleY; // been adjusted to be relative to element
